...
from light_processor import LightDocumentProcessor


logger = CustomLogger("prepare_doc_redis")
logflag = os.getenv("LOGFLAG", False)
use_light_processing = os.getenv("USE_LIGHT_PROCESSING", "true").lower() == "true"
...
def ingest_data_to_redis(doc_path: DocPath):
    ...
    if use_light_processing:
        # Use lightweight processing without heavy AI models
        logger.info("[ ingest data ] Using light processing mode")
        light_processor = LightDocumentProcessor()
        light_processor.chunk_size = doc_path.chunk_size
        light_processor.chunk_overlap = doc_path.chunk_overlap
        
        document_chunks = light_processor.process_document(path)
        chunks = [chunk['text'] for chunk in document_chunks]
        
        logger.info(f"[ ingest data ] Light processing completed. Created {len(chunks)} chunks.")
    else:
        # Original heavy processing with AI models
        logger.info("[ ingest data ] Using heavy AI model processing")
        tree = Tree(path)
        tree_parser = TreeParser()
